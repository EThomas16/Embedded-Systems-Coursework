...
void snakeRefresh(void) {
	...
	GLCD_DisplayString(headY, headX, __FI, &snakeArray[3]);
	delay(1);
	GLCD_ClearLn(headX, Black);
	for (int i=1; i<20; i++) {
		GLCD_DisplayString(headY, i, __FI, " ");
	}
}

void snakeMovement(int snakeDirection) {
...
int main (void) {
	...
	for (int i=0; i<sizeof(snakeArray); i++) {
			GLCD_DisplayString(5, (8+i), __FI, &snakeArray[i]);
	}
