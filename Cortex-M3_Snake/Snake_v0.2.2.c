 ...
 *		Continuous movement required, maybe persistent check
 */
...
void snakeDrawing(void) {
    ...
    for (int i=0; i<sSnake.tailLen; i++) {
        GLCD_DisplayString(tailX[i], tailY[i], __FI, &sSnake.shape);
    }   
}
...
int main (void) {
	...
	GLCD_SetTextColor(White);
	GLCD_DisplayString(0, 6, __FI, "CIS-3108");
	GLCD_DisplayString(1, 5, __FI, "Snake-Game");
#endif // __USE_LCD
	...
	while(TRUE) {
		if (!gameOver) {
			...
			switch(joystick_int) {
				case JOYSTICK_UP:
					GLCD_DisplayString(0, 15, __FI, "up");
					sSnake.xD=0;
					sSnake.yD=-1;
					...
					break;
				case JOYSTICK_DOWN:
					GLCD_DisplayString(0, 15, __FI, "down");
					sSnake.xD=0;
					sSnake.yD=1;
					snakeLogic(sSnake.xD, sSnake.yD);
					joystick_int=0;
					break;
				case JOYSTICK_LEFT:
					GLCD_DisplayString(0, 15, __FI, "left");
					sSnake.xD=-1;
					sSnake.yD=0;
					snakeLogic(sSnake.xD, sSnake.yD);
					joystick_int=0;
					break;
				case JOYSTICK_RIGHT:
					GLCD_DisplayString(0, 15, __FI, "right");
					sSnake.xD=1;
					sSnake.yD=0;
					snakeLogic(sSnake.xD, sSnake.yD);
					joystick_int=0;
