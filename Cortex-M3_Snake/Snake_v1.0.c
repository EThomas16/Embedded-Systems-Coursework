...
int width = 20;
int height = 10;
//Sets the maximum length of the tail
int tailX[100];
int tailY[100];

struct food {
	//The food is initialised with a shape and coordinates 
	//This allows for manipulation of its position during runtime
    unsigned char shape;
    int x;
    int y;
};
...
struct snake {
    ...
    int x;
    int y;
	//As well as having x and y co-ords, the x and y-direction of the snake is stored
	//This is used to check which direction the snake is going in
    int xD;
    int yD;
...
int snakeInit(void) {
	...
	sSnake.x = 10;
	sSnake.y = 5;
	//State of the snake is initialised as stationary (0) to be changed to the joystick movement values when running
	sSnake.xD = STATIONARY;
	sSnake.yD = STATIONARY;
	//Uses the rand() method, seeded in the main method, to randomly generate the food's location
	sFood.x = rand() % width;
	sFood.y = rand() % height;
	//Snake starts as just the head, so the tail doesn't have a length yet
	sSnake.tailLen = 0;
	sSnake.speed = 300;
	//If LEDs were turned on during the last game, they are all turned off
	for (int i = 0; i < 8; i++){
		LED_Off(i);
	}
}
...
void snakeDrawing(void) {
		//Responsible for drawing the snake to the LCD screen
		unsigned char tailScore[2] = { 0, 0 };
		
		GLCD_DisplayString(0, 7, __FI, "Score: ");
		//Draws a single character for the score if less than 9, to prevent overlap issues with displaying the current score
		if (sSnake.tailLen <= 9) {
			tailScore[0] = sSnake.tailLen + '0';
			GLCD_DisplayString(0, 14, __FI, &tailScore[0]);
		}
		//Otherwise, if the length requires two characters to be displayed this is accounted for
		else if (sSnake.tailLen > 9) {
			int val = sSnake.tailLen;
			//Loops to display each character for tens and single digits column
			for (int i=0; i<2; i++) {
				//Get remainder of value to calculate the single digits column
				int val2 = val % 10;
				tailScore[i] = val2 + '0';
				//Value divided by ten for second iteration
				val /=10;
			}
			//Take second character of number and place it next to first
			GLCD_DisplayString(0, 14, __FI, &tailScore[1]);
			GLCD_DisplayString(0, 15, __FI, &tailScore[0]);
			//This space is blank to overwrite the extra digit to ensure the score displays properly
			GLCD_DisplayString(0, 16, __FI, " ");
		}
    GLCD_DisplayString(sSnake.y, sSnake.x, __FI, &sSnake.shape);
    GLCD_DisplayString(sFood.y, sFood.x, __FI, &sFood.shape);
		//Displays the entire snake every tick
...
void snakeMenu(void) {
	//Method for initialising the menu
	GLCD_DisplayString(0, 5, __FI, "CIS-3108");
	GLCD_DisplayString(1, 4, __FI, "Snake-Game");
	GLCD_DisplayString(3, 2, __FI, "Press joystick to");
	GLCD_DisplayString(4, 3, __FI, "begin the game!");
}

void snakeLogic(xDir, yDir) {
    //Performs the logical operations to move the snake, read jostick inputs etc.
		if (sSnake.xD == STATIONARY && sSnake.yD == STATIONARY) {
			return;
		}
		//Sets the previous x and y values
		//Snake's body co-ordinates stored in arrays tailX and tailY
    int lastX = tailX[0];
    int lastY = tailY[0];
    int lastX_, lastY_;
		//Current tail set as the head's x and y
    tailX[0] = sSnake.x;
    tailY[0] = sSnake.y;
		if (sSnake.tailLen == 1){
			GLCD_Clear(Black);
		} else {
			//Loops through length of the snake and clears the tail positions
			for (int i = 0; i <= sSnake.tailLen; i++){
				GLCD_DisplayString(tailY[i], tailX[i], __FI, " ");
			}		
		}
		//Loops through the snake and sets its x and y co-ordinates
    for (int i=1; i<sSnake.tailLen; i++) {
		...
		//Checks the direction of the snake and increments the snake's position accordingly
		if (xDir == 0 && yDir == -1) {
			sSnake.y--;
		}
		...
		//If the snake goes off the edge of the screen, it is moved to the opposing side of the screen to create a continuous movement effect
    //This is done by setting the value of the snake's head to be the opposite side of the screen to that which the snake went off
		if (sSnake.x >= width) {
        sSnake.x = 0;
    } else if (sSnake.x < 0) {
        sSnake.x = width - 1;
    ...
    if (sSnake.x == sFood.x && sSnake.y == sFood.y) {
			//When the snake eats food, a new food location is generated and the length of the tail is increased
        sFood.x = rand() % width;
        sFood.y = rand() % height;
        sSnake.tailLen++;
				if (sSnake.tailLen <= 8) {
					//As well as the length of the tail increasing the LEDs light up and the speed of the snake increases
					LED_On(sSnake.tailLen-1);
					sSnake.speed = sSnake.speed - 25;
				}
				//When the food is eaten the speaker makes a noise
				for (int c=0; c<2; c++) {
					generate_Tone_On_Speaker(c*1000, 50);
				}
...
int main (void) {
	...
	AD_done = 0;
	//Bit shifts unsigned long of 1 22 bits right
	ADC1->CR2 |= (1UL << 22);
	while(TRUE) {
		//If analogue digital converted is set up, RNG is seeded
		if (AD_done) {
			srand(AD_last);
			break;
		}
	...
	user_btn_int = 0;
	joystick_int = 0;
	//Set for checking if the game has begun, indicated by the user pressing the joystick down
	int beginGame = FALSE;
	snakeInit();
	snakeMenu();
	//While the game has not started
	while(!beginGame) {
		//If button pressed, set to true
		if (joystick_int == JOYSTICK_SELECT) {
			joystick_int = 0;
			beginGame = TRUE;
		}
	}
	//Clears the menu from the screen before the game begins
	GLCD_Clear(Black);
	
	while(TRUE) {
		//Checks if the game is not over every tick
		if (!gameOver) {
			//Checks for joystick vectored interrupts to determine the direction of the snake
			switch(joystick_int) {
				case JOYSTICK_UP:
					//Sets the x and y directions of the snake
					sSnake.xD=0;
					sSnake.yD=-1;
					break;
				case JOYSTICK_DOWN:
			...
			//Resets the joystick's current position
			joystick_int = 0;
			//Can alter this to increase/decrease difficulty
			delay(sSnake.speed);
			//Uses the values set by the joystick interrupts to turn the snake
			...
			GLCD_DisplayString(5, 6, __FI, "GAME OVER");
			//Waits for the uesr button press before resetting the game
			while (user_btn_int == 0) {	
				gameOver = FALSE;		
			}
			//Clears the screen and re-initialises the game
			GLCD_Clear(Black);
			snakeInit();
		}
	}
